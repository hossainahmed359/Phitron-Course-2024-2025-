...
SELECT
    ...
    CONCAT (m.first_name, ' ', m.last_name) AS 'Manager Full name'
FROM
    employees AS e
...
SELECT
    ...
    e1.salary,
    e2.avg_salary,
    e2.avg_salary - e1.salary AS 'less from avg salary',
    d.department_name
FROM
    employees AS e1
    LEFT JOIN t_avg AS e2 ON e1.department_id = e2.department_id
    LEFT JOIN departments AS d ON e1.department_id = d.department_id;

-- QUESTION 4 : Write a query to show the departments which has a minimum salary is greater than 5000
--1 
SELECT
    department_name,
    MIN(salary)
